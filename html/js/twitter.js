...
$(document).on('click', '.twitter-header-tab', function(e){
    ...
    if (PressedTwitterTab !== CurrentTwitterTab) {
        ...
        if (PressedTwitterTab === "twitter-mentions") {
            $.post('http://reborn_phone/ClearMentions');
        }
...
$(document).on('click', '#atualizar-tweets', function(e){
    e.preventDefault();
    //console.log('atualizando tweets')
});

$(document).on('click', '.twitter-new-tweet', function(e){
    e.preventDefault();

    // Reborn.Phone.Animations.TopSlideDown(".twitter-new-tweet-tab", 450, 0);
    $(".twitter-new-tweet-tab").fadeIn('fast');
});

Reborn.Phone.Notifications.LoadTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        $(".twitter-home-tab").html("");
        $.each(Tweets, function(i, Tweet){
            var TwtMessage = Reborn.Phone.Functions.FormatTwitterMessage(Tweet.message);
            var today = new Date();
            ...
            var TweetElement = '<div class="twitter-tweet" data-twthandler="@'+TwitterHandle.replace(" ", "_")+'">'+
                '<div class="tweet-tweeter"><strong>'+Tweet.firstName+' '+Tweet.lastName+'</strong> &nbsp;<span>@'+TwitterHandle.replace(" ", "_")+' &middot; '+TimeAgo+'</span></div>'+
                '<div class="tweet-message">'+TwtMessage+'</div>'+
                '<div class="twt-img" style="top: 1.5vh;"><img src="' + PictureUrl + '" class="tweeter-image"></div>' +
...
$("#tweet-new-message").keyup(function(){
    $(".caractere-count").text($(this).val().length);
});
...
Reborn.Phone.Notifications.LoadMentionedTweets = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet){
            ...
            '<div class="twitter-tweet">'+
                ...
                '<div class="tweet-message">'+TwtMessage+'</div>'+
            '<div class="twt-img" style="top: 1.5vh;"><img src="'+PictureUrl+'" class="tweeter-image"></div></div>';
...
Reborn.Phone.Functions.FormatTwitterMessage = function(TwitterMessage) {
...
$(document).on('click', '#send-tweet', function(e){
    ...
    if (TweetMessage != "") {
        ...
        $.post('http://reborn_phone/PostNewTweet', JSON.stringify({
            Message: TweetMessage,
            Date: CurrentDate,
            Picture: Reborn.Phone.Data.MetaData.profilepicture
        }), function(Tweets){
            Reborn.Phone.Notifications.LoadTweets(Tweets);
        });
        ...
        // Reborn.Phone.Animations.TopSlideUp(".twitter-new-tweet-tab", 450, -120);
        $(".twitter-new-tweet-tab").fadeOut('fast');
    } else {
        // Reborn.Phone.Notifications.Add("fab fa-twitter", "Twitter", "Please enter a message!", "#1DA1F2");
        NotifyInside('Twitter','Twitter','twitter','Por favor coloque alguma mensagem!')
    }
});
...
$(document).on('click', '.hashtag-tag-text', function(e){
    ...
    if (!HashtagOpen) {
        ...
        $.post('http://reborn_phone/GetHashtagMessages', JSON.stringify({hashtag: Hashtag}), function(HashtagData){
            Reborn.Phone.Notifications.LoadHashtagMessages(HashtagData.messages);
        });
...
function CopyMentionTag(elem) {
    ...
    // Reborn.Phone.Notifications.Add("fab fa-twitter", "Twitter", $(elem).data('mentiontag') + " copied!", "rgb(27, 149, 224)", 1250);
    NotifyInside('Twitter','Twitter','twitter',$(elem).data('mentiontag') + " mencionado!")
    document.execCommand("copy");
    $temp.remove();
}

Reborn.Phone.Notifications.LoadHashtags = function(hashtags) {
    if (hashtags !== null) {
        ...
        $.each(hashtags, function(i, hashtag){
            ...
            if (hashtag.messages.length >= MinimumTrending) {
                Elem = '<div class="twitter-hashtag" id="tag-'+hashtag.hashtag+'"><div class="twitter-hashtag-status">Em Alta</div> <div class="twitter-hashtag-tag">#'+hashtag.hashtag+'</div> <div class="twitter-hashtag-messages">'+hashtag.messages.length+' '+TweetHandle+'</div> </div>';
            } else {
                Elem = '<div class="twitter-hashtag" id="tag-'+hashtag.hashtag+'"><div class="twitter-hashtag-status">Estao falando sobre</div> <div class="twitter-hashtag-tag">#'+hashtag.hashtag+'</div> <div class="twitter-hashtag-messages">'+hashtag.messages.length+' '+TweetHandle+'</div> </div>';
            }
...
Reborn.Phone.Notifications.LoadHashtagMessages = function(Tweets) {
    Tweets = Tweets.reverse();
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
...
$(document).on('click', '.twitter-hashtag', function(event){
    ...
    Reborn.Phone.Notifications.LoadHashtagMessages(TweetData.messages);
