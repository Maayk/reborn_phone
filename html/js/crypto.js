...
function SetupCryptoData(Crypto) {
    ...
    $(".crypto-action-page-wallet").html("Portfolio: "+CryptoData.Portfolio+" Bitcoin('s)");
    ...
    if (CryptoData.History.length > 0) {
        ...
        $.each(CryptoData.History, function(i, change){
            ...
            var Element =   '<div class="cryptotab-course-block">' +
                                ...
                                '<span class="cryptotab-course-block-title">Exchange rate</span>' +
...
$(document).on('click', '#buy-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (Reborn.Phone.Data.PlayerData.money.bank >= Price) {
            $.post('http://reborn_phone/BuyCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    Reborn.Phone.Data.PlayerData.money.bank = parseInt(Reborn.Phone.Data.PlayerData.money.bank) - parseInt(Price);
                    Reborn.Phone.Notifications.Add("fas fa-university", "COABank", "There was &dollar; "+Price+" taken off!", "#badc58", 2500);
                } else {
                    Reborn.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You do not have enough money..", "#badc58", 1500);
                }
            });
        } else {
            Reborn.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You do not have enough money..", "#badc58", 1500);
        ...
        Reborn.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Fill in everything!", "#badc58", 1500);
    }
});
...
$(document).on('click', '#sell-crypto', function(e){
    ...
    if ((Coins !== "") && (Price !== "")) {
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('http://reborn_phone/SellCrypto', JSON.stringify({
                Coins: Coins,
                Price: Price,
            }), function(CryptoData){
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    Reborn.Phone.Data.PlayerData.money.bank = parseInt(Reborn.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    Reborn.Phone.Notifications.Add("fas fa-university", "COABank", "There is &dollar; "+Price+" credited!", "#badc58", 2500);
                } else {
                    Reborn.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Bitcoins ..", "#badc58", 1500);
                }
            });
        } else {
            Reborn.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Bitcoins ..", "#badc58", 1500);
    ...
    if ((Coins !== "") && (WalletId !== "")) {
        if (CryptoData.Portfolio >= Coins) {
            if (WalletId !== CryptoData.WalletId) {
                $.post('http://reborn_phone/TransferCrypto', JSON.stringify({
                    Coins: Coins,
                    WalletId: WalletId,
                }), function(CryptoData){
                    if (CryptoData == "notenough") {
                        ...
                        Reborn.Phone.Notifications.Add("fas fa-university", "Crypto", "This Wallet ID does not exist!", "#badc58", 2500);
                    } else {
                        UpdateCryptoData(CryptoData)
                        CloseCryptoPage()
                        Reborn.Phone.Notifications.Add("fas fa-university", "Crypto", "You Have "+Coins+" transferred to"+WalletId+"!", "#badc58", 2500);
                    }
                });
            } else {
                Reborn.Phone.Notifications.Add("fas fa-university", "Crypto", "You cannot transfer to yourself ..", "#badc58", 2500);
            }
        } else {
            Reborn.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Bitcoins ..", "#badc58", 1500);
        }
